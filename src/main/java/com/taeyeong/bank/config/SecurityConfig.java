...
import com.taeyeong.bank.util.CustomResponseUtil;
...
public class SecurityConfig {
    ...
    // JWT 서버를 만들 예정, Session 사용하지 않음.
    @Bean
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        log.debug("디버그: filterChain 빈 등록됨");
        http.headers().frameOptions().disable(); // iframe 허용 안 함
        http.csrf().disable(); // enable이면 postman 작동 안 함
        ...
        http.authorizeRequests()
            ...
            .antMatchers("/api/admin/**").hasRole("" + UserEnum.ADMIN) // 최근 공식문서에서는 ROLE_ 안붙여도 된다고 함
